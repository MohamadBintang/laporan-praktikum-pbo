...
public class Anggota1841720007Bintang {
    ...
    public Anggota1841720007Bintang getByIdBintang(int id) {
        Anggota184172007Bintang agt = new Anggota1841720007Bintang();
    ...
    public ArrayList<Anggota1841720007Bintang> getByNamaAndAlamatAndTeleponBintang(String nama, String alamat, String telepon) {
        ArrayList<Anggota1841720007Bintang> ListAnggota = new ArrayList();
        ResultSet rs;
        if ((nama.trim().length() > 0) && (alamat.trim().length() > 0) && (telepon.trim().length() > 0)) {
            rs = DBHelper1841720007Bintang.selectQueryBintang("SELECT * FROM anggota WHERE nama = '" + nama + "'  AND alamat = '" + alamat + "'  AND telepon = '" + telepon + "'");

        } else if ((nama.trim().length() > 0) && (alamat.trim().length() > 0) && (telepon.trim().length() == 0)) {
            rs = DBHelper1841720007Bintang.selectQueryBintang("SELECT * FROM anggota WHERE nama = '" + nama + "'  AND alamat = '" + alamat + "'");
        } else if ((nama.trim().length() > 0) && (alamat.trim().length() == 0)) {
            rs = DBHelper1841720007Bintang.selectQueryBintang("SELECT * FROM anggota WHERE nama = '" + nama + "'");

        } else {
            rs = DBHelper1841720007Bintang.selectQueryBintang(
                    "SELECT * FROM anggota "
                    + "WHERE alamat = '" + alamat + "'  ");
        }
        try {
            while (rs.next()) {
                Anggota1841720007Bintang ang = new Anggota1841720007Bintang();
                ang.setIdanggotaBintang(rs.getInt("idanggota"));
                ang.setNamaBintang(rs.getString("nama"));
                ang.setAlamatBintang(rs.getString("alamat"));
                ang.setTeleponBintang(rs.getString("telepon"));
                ListAnggota.add(ang);
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void saveBintang() {
        if (getByIdBintang(mIdanggota).getIdanggotaBintang() == 0) {
